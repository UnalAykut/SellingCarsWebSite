...
if (isset($_POST['motor_id']) && isset($_POST['is_favorite'])) {
    ...
    if ($isFavorite == "false") {
        ...
        $checkQuery = "SELECT * FROM favoriler WHERE id = '$motorId'";
        ...
        // Favorilerden sil
        $deleteQuery = "DELETE FROM favoriler WHERE id = '$motorId'";
...
$(document).ready(function() {
    $('.star').on('click', function() {
        var motorId = $(this).attr('data-motor-id');
...
<section class="services" id="services">
    ...
    <h1 class="heading"><span>Motorsikletler</span> </h1>
    ...
    <div class="box-container">
        ...
        $sql = "SELECT id, image, name, price, year, transmission, fuel, top_speed, is_favorite FROM motorlar";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                <div class="box">
                    <div class="starbox">
                        <input id="star' . $row['id'] . '" class="star" type="checkbox" title="bookmark page" data-motor-id="' . $row['id'] . '">
        
                        <label for="star' . $row['id'] . '" title="Amazing"></label>
                    </div>
                            <img src="image/' . $row["image"] . '" alt="' . $row["name"] . '" class="motor-image">
                            <h3>' . $row["name"] . '</h3>
                            ...
                            <a href="motoral2.php?motor_id=' . $row["id"] . '" class="btn">Satın Al</a>
                        </div>';
                    }
                } else {
